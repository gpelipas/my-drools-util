...
public class DynamicBeanBuilder {
	...
	private CtMethod generateGetter(CtClass hostClz, String fieldName, Class<?> fieldTypeCls) throws CannotCompileException {
	...
	private CtMethod generateSetter(CtClass hostClz, String fieldName, Class<?> fieldTypeCls) throws CannotCompileException {
		final String methodName = "set" + fieldName.substring(0, 1).toUpperCase() + fieldName.substring(1);

		StringBuilder sb = new StringBuilder();
		sb.append("public void ").append(methodName).append("(").append(fieldTypeCls.getName()).append(" fldName").append(") { ");
